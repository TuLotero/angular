...
export enum DriverReadyState {
  ...
  // The SW does not have a clean installation of the latest version of the
  // app, but older cached versions are safe to use so long as they don't try
  // to fetch new dependencies. This is a degraded state.
  EXISTING_CLIENTS_ONLY,

  // The SW has decided that caching is completely unreliable, and is forgoing
  // request handling until the next restart.
  SAFE_MODE,
}
...
export class Driver implements Debuggable, UpdateSource {
  ...
  constructor(
    ...
    this.scope.addEventListener('install', (event) => {
      // SW code updates are separate from application updates, so code updates
      // are almost as straightforward as restarting the SW. Because of this,
      // it's always safe to skip waiting until application tabs are closed,
      // and activate the new SW version immediately.
      event !.waitUntil(this.scope.skipWaiting());
    });

    // The activate event is triggered when this version of the service worker
    // is first activated.
    this.scope.addEventListener('activate', (event) => {
      // As above, it's safe to take over from existing clients immediately,
      // since the new SW version will continue to serve the old application.
      ...
      // in the context of the 'activate' event, waitUntil() here would cause
      // fetch events to block until initialization completed. Thus, the SW
      // does a postMessage() to itself, to schedule a new event loop iteration
      // with an entirely separate event context. The SW will be kept alive by
      // waitUntil() within that separate context while initialization
      // proceeds, while at the same time the activation event is allowed to
      // resolve and traffic starts being served.
    ...
    this.scope.addEventListener('pushsubscriptionchange',
      (event) => this.onPushSubscriptionChange(event !));
    this.scope.addEventListener('notificationclick',
      (event) => this.onNotificationClick(event !));
  ...
  private onFetch(event: FetchEvent): void {
    ...
    if (this.adapter.parseUrl(req.url, this.scope.registration.scope).path ===
        '/ngsw/state') {
    ...
    // If the SW is in a broken state where it's not safe to handle requests at
    // all, returning causes the request to fall back on the network. This is
    // preferred over `respondWith(fetch(req))` because the latter still shows
    // in DevTools that the request was handled by the SW. TODO: try to handle
    // DriverReadyState.EXISTING_CLIENTS_ONLY here.
    if (this.state === DriverReadyState.SAFE_MODE) {
      // Even though the worker is in safe mode, idle tasks still need to
      // happen so things like update checks, etc. can take place.
      event.waitUntil(this.idle.trigger());
      return;
    }

    // When opening DevTools in Chrome, a request is made for the current URL
    // (and possibly related resources, e.g. scripts) with `cache:
    // 'only-if-cached'` and `mode: 'no-cors'`. These request will eventually
    // fail, because `only-if-cached` is only allowed to be used with `mode:
    // 'same-origin'`. This is likely a bug in Chrome DevTools. Avoid handling
    // such requests. (See also
    // https://github.com/angular/angular/issues/22362.) TODO(gkalpak): Remove
    // once no longer necessary (i.e. fixed in Chrome DevTools).
    if ((req.cache as string) === 'only-if-cached' && req.mode !==
        'same-origin') {
      // Log the incident only the first time it happens, to avoid spamming the
      // logs.
    ...
    // Past this point, the SW commits to handling the request itself. This
    // could still fail (and result in `state` being set to `SAFE_MODE`), but
    // even in that case the SW will still deliver a response.
  ...
  private onMessage(event: ExtendableMessageEvent): void {
    ...
    // Initialization is the only event which is sent directly from the SW to
    // itself, and thus `event.source` is not a Client. Handle it here, before
    // the check for Client sources.
    ...
    // Only messages from true clients are accepted past this point (this is
    // essentially a typecast).
  ...
  private async onPushSubscriptionChange(event: PushSubscriptionChangeEvent) {
    ...
    if (current) {
      ...
      if (manifest.push) {
        ...
        if (!response.ok) {
          this.debugger.log('Tried to upload new push subscription but server' +
  ...
  private async handleMessage(msg: MsgAny & { action: string },
                              from: Client): Promise<void> {
  ...
  private async reportStatus(client: Client,
                             promise: Promise<void>,
                             nonce: number): Promise<void> {
  ...
  async updateClient(client: Client): Promise<void> {
    ...
    // Look up the application data associated with the existing version. If
    // there isn't any, fall back on using the hash.
  ...
  private async handleFetch(event: FetchEvent): Promise<Response> {
    // Since the SW may have just been started, it may or may not have been
    // initialized already. this.initialized will be `null` if initialization
    // has not yet been attempted, or will be a Promise which will resolve
    // (successfully or unsuccessfully) if it has.
    if (this.initialized === null) {
      // Initialization has not yet been attempted, so attempt it. This should
      // only ever happen once per SW instantiation.
      this.initialized = this.initialize();
    }

    // If initialization fails, the SW needs to enter a safe state, where it
    // declines to respond to network requests.
    try {
      ...
      this.stateMessage =
        `Initialization failed due to error: ${errorToString(e)}`;

      // Even though the driver entered safe mode, background tasks still need
      // to happen.
      event.waitUntil(this.idle.trigger());

      // Since the SW is already committed to responding to the currently
      // active request, respond with a network fetch.
    ...
    // Decide which version of the app to use to serve this request. This is
    // asynchronous as in some cases, a record will need to be written to disk
    // about the assignment that is made.
    ...
    try {
      // Handle the request. First try the AppVersion. If that doesn't work,
      ...
      if (err.isCritical) {
        ...
        await this.versionFailed(appVersion,
          err,
    ...
    // The AppVersion will only return null if the manifest doesn't specify
    // what to do about this request. In that case, just fall back on the
    // network.
    if (res === null) {
      event.waitUntil(this.idle.trigger());
      return this.safeFetch(event.request);
    ...
    // Trigger the idle scheduling system. The Promise returned by trigger()
    // will resolve after a specific amount of time has passed. If trigger()
    // hasn't been called again by then (e.g. on a subsequent request), the
    // idle task queue will be drained and the Promise won't resolve until that
  ...
  private async initialize(): Promise<void> {
    // On initialization, all of the serialized state is read out of the
    // 'control' table. This includes: - map of hashes to manifests of
    // currently loaded application versions - map of client IDs to their
    // pinned versions - record of the most recently fetched manifest hash  If
    // these values don't exist in the DB, then this is the either the first
    // time the SW has run or the DB state has been wiped or is inconsistent.
    // In that case, load a fresh copy of the manifest and reset the state from
    // scratch.

    // Open up the DB table.
    const table = await this.db.open('control');

    // Attempt to load the needed state from the DB. If this fails, the catch
    // {} block will populate these variables with freshly constructed values.
    let manifests: ManifestMap, assignments: ClientAssignments,
      latest: LatestEntry;
    try {
      ...
      // Successfully loaded from saved state. This implies a manifest exists,
      // so the update check needs to happen in the background.
    ...
    // At this point, either the state has been loaded successfully, or fresh
    // state with a new copy of the manifest has been produced. At this point,
    // the `Driver` can have its internals hydrated from the state.

    // Initialize the `versions` map by setting each hash to a new `AppVersion`
    // instance for that manifest.
    Object.keys(manifests).forEach((hash: ManifestHash) => {
      const manifest = manifests[hash];

      // If the manifest is newly initialized, an AppVersion may have already
      // been created for it.
      if (!this.versions.has(hash)) {
        this.versions.set(
          hash,
          new AppVersion(this.scope,
            this.adapter,
            this.db,
            this.idle,
            manifest,
    ...
    // Map each client ID to its associated hash. Along the way, verify that
    // the hash is still valid for that client ID. It should not be possible
    // for a client to still be associated with a hash that was since removed
    // from the state.
    ...
    // Finally, wait for the scheduling of initialization of all versions in
    // the
    // manifest. Ordinarily this just schedules the initializations to happen
    // during the next idle period, but in development mode this might actually
    // wait for the full initialization. If any of these initializations fail,
    // versionFailed() will be called either synchronously or asynchronously to
    // handle the failure and re-map clients.
  ...
  private lookupVersionByHash(hash: ManifestHash,
  ...
  private async assignVersion(event: FetchEvent): Promise<AppVersion | null> {
    // First, check whether the event has a (non empty) client ID. If it does,
    // the version may already be associated.
    ...
    if (clientId) {
      ...
      if (this.clientVersionMap.has(clientId)) {
        ...
        // Ordinarily, this client would be served from its assigned version.
        // But, if this request is a navigation request, this client can be
        // updated to the latest version immediately.
        if (this.state === DriverReadyState.NORMAL && hash !==
            this.latestHash &&
        ...
        // This is the first time this client ID has been seen. Whether the SW
        // is in a state to handle new clients depends on the current readiness
        // state, so check that first.
        if (this.state !== DriverReadyState.NORMAL) {
          // It's not safe to serve new clients in the current state. It's
          // possible that this is an existing client which has not been mapped
          // yet (see below) but even if that is the case, it's invalid to make
          // an assignment to a known invalid version, even if that assignment
          // was previously implicit. Return undefined here to let the caller
          // know that no assignment is possible at this time.
          return null;
        }

        // It's safe to handle this request. Two cases apply. Either:
        // 1) the browser assigned a client ID at the time of the navigation
        // request, and this is truly the first time seeing this client, or 2)
        // a navigation request came previously from the same client, but with
        // no client ID attached. Browsers do this to avoid creating a client
        // under the origin in the event the navigation request is just
        // redirected.  In case 1, the latest version can safely be used. In
        // case 2, the latest version can be used, with the assumption that the
        // previous navigation request was answered under the same version.
        // This assumption relies on the fact that it's unlikely an update will
        // come in between the navigation request and requests for subsequent
      ...
      // No client ID was associated with the request. This must be a
      // navigation request for a new client. First check that the SW is
      // accepting new clients.
      if (this.state !== DriverReadyState.NORMAL) {
        return null;
      }

      // Serve it with the latest version, and assume that the client will
      // actually get associated with that version on the next request.
  ...
  private async fetchLatestManifest(ignoreOfflineError = false): Promise<Manifest | null> {
    const res =
      await this.safeFetch(this.adapter.newRequest('ngsw.json?ngsw-cache-bust=' +
                                                   Math.random()));
    if (!res.ok) {
      if (res.status === 404) {
   ...
   */
  private async scheduleInitialization(appVersion: AppVersion,
                                       latest: boolean): Promise<void> {
    const initialize = async () => {
      try {
        await appVersion.initializeFully();
      } catch (err) {
        this.debugger.log(err,
    ...
    this.idle.schedule(`initialization(${appVersion.manifestHash})`,
      initialize);
  }

  private async versionFailed(appVersion: AppVersion,
                              err: Error,
                              latest: boolean): Promise<void> {
    // This particular AppVersion is broken. First, find the manifest hash.
    const broken =
      Array.from(this.versions.entries())
        .find(([hash, version]) => version === appVersion);
    if (broken === undefined) {
    ...
    // The action taken depends on whether the broken manifest is the active
    // (latest) or not. If so, the SW cannot accept new clients, but can
    // continue to service old ones.
    if (this.latestHash === brokenHash || latest) {
      // The latest manifest is broken. This means that new clients are at the
      // mercy of the network, but caches continue to be valid for previous
      // versions. This is unfortunate but unavoidable.
      this.state = DriverReadyState.EXISTING_CLIENTS_ONLY;
      this.stateMessage =
        `Degraded due to failed initialization: ${errorToString(err)}`;
      ...
      // possible remedy is to stop serving the older version and go to the
      // network. Figure out which clients are affected and put them on the
      // latest.
      ...
      affectedClients.forEach(
        clientId => this.clientVersionMap.set(clientId, this.latestHash !));
    }
    await this.sync();
  }

  private async setupUpdate(manifest: Manifest, hash: string): Promise<void> {
    const newVersion = new AppVersion(this.scope,
      this.adapter,
      this.db,
      this.idle,
      manifest,
    ...
    // It's always safe to update from a version, even a broken one, as it will
    // still only have valid resources cached. If there is no latest version,
    // though, this update will have to install as a fresh version.
    if (this.latestHash !== null) {
    ...
    // Cause the new version to become fully initialized. If this fails, then
    // the version will not be available for use.
  ...
  private async sync(): Promise<void> {
    ...
    // Record the latest entry. Since this is a sync which is necessarily
    // happening after initialization, latestHash should always be valid.
  ...
  async cleanupCaches(): Promise<void> {
    // Query for all currently active clients, and list the client ids. This
    // may skip some clients in the browser back-forward cache, but not much
    ...
    // activeClients from this list will result in the set of client ids which
    // are being tracked but are no longer used in the browser, and thus can be
    // cleaned up.
    ...
    // Next, determine the set of versions which are still used. All others can
    // be removed.
    const usedVersions = new Set<string>();
    this.clientVersionMap.forEach((version, _) => usedVersions.add(version));

    // Collect all obsolete versions by filtering out used versions from the
    // set of all versions.
    ...
    await obsoleteVersions.reduce(async (previous, version) => {
      ...
      try {
        ...
        // Oh well? Not much that can be done here. These caches will be
        // removed when the SW revs its format version, which happens from time
        // to time.
  ...
  lookupResourceWithHash(url: string, hash: string): Promise<Response | null> {
    return Array
    // Scan through the set of all cached versions, valid or otherwise. It's
    // safe to do such lookups even for invalid versions as the cached version
    // of a resource will have the same hash regardless.
      .from(this.versions.values())
      // Reduce the set of versions to a single potential result. At any point
      // along the reduction, if a response has already been identified, then
      // pass it through, as no future operation could change the response. If
      // no response has been found yet, keep checking versions until one is or
      // until all versions have been exhausted.
      .reduce(async (prev, version) => {
        // First, check the previous result. If a non-null result has been
        // found already, just return it.
        if (await prev !== null) {
          return prev;
        }
  ...
  private mergeHashWithAppData(manifest: Manifest,
                               hash: string): { hash: string, appData: Object } {
    return {
      hash,
